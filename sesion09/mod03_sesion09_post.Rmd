...
+ ¿Tienen nociones de qué es análisis supervisado y no-supervisado?

+ Aunque van a ver con detalle esto en los siguientes módulos (5 y 6) vamos a dar una intuición

+ En aprendizaje supervisado tiene un conjunto de variables, se ha identificado una de ellas como especial y se esta buscando entender como las otras variables ayudan a explicar a esta variable especial.

+ **Notación** $Y$: Es esta variable especial. Le llamamos variable objetivo, variable respuesta o variable supervisora. Al resto de las variables las denotamos como $X_1,X_2,\ldots,X_p$ y las llamamos variables explicativas.

+ Con esta notación, en aprendizaje supervisado, se pretende buscar una función $g$ que relacione a la variable respuesta con las variable explicativas, i.e. $Y = g(X_1,X_2,\ldots,X_n)$

+ Dicho esto, ¿han visto alguna técnica de aprendizaje supervisado? Sí

+ Ejemplo clásico por excelencia es la regresión lineal
$$Y = \beta_0 + \beta_1X_1 + \beta_2X_2 + \ldots + \beta_pX_p$$

+ Por analogía, en el aprendizaje no-supervisado NO se tiene una variable más especial que las otras, simplemente se tienen variables $X_1,X_2,\ldots,X_p$ y lo que se busca es entender la estructura intrínseca entre éstas.

+ Ejemplos: Clusterización (meter en grupitos a los datos), reducción de dimensionalidad i.e. quitar información redundante (las técnicas clásicas son Análisis de Componentes Principales (Principal Component Analysis) y Vecinos $t$ estocásticos).

+ Como esto sólo pretende ser una ayuda para hacer el EDA, sólo introduciremos PCA 
...
# Manejo y operaciones de dataframes
library(tibble)
library(dplyr)
library(tidyr)

# La carnita de la sesión, son las PCA y clusterización
library(factoextra)
library(cluster)
```
...
+ En principio, sólo vamos a trabajar con columnas numéricas.
...
2. Para visualizar datos de muchas dimensiones físicas (por ejemplo, en 2D o 3D [no la recomiendo mucho]), i.e. para intentar visualizar algo en $n$ dimensiones, en sólo 2 dimensiones.
...
+ **Importante**: Es un método lineal, i.e. puede no funcionar bien con relaciones no lineales. Recordatorio: correlación = relación lineal.

+ **Cuidado/Doloroso**: Interpretar las componentes principales puede ser difícil pues representan dimensiones abstractas.... ahorita veremos a qué me refiero.
...
+ Para las personas que tienen un contexto analítico más formal... pues simplemente un cambio de coordenadas y una proyección (Algebra Lineal, Geo. Analítica 2, Cálculo III)
...
+ Es decir, me quedaré con $$CP_1,CP_2,\ldots, CP_k$$ donde $k<p$... es precisamente esto la reducción de dimensionalidad.
...
+ Una de las partes más importantes en PCA es encontrar los eigenvalores y eigenvectores de la **matriz de covarianzas**

+ Resultado: Es simétrica, sus eigenvalores son reales y no negativos, y sus eigenvectores son ortogonales y normalizados... i.e. los eigenvectores son perpendiculares entre sí.... Recuerdos del curso de Alg. Lineal
...
+ Es info de los puntajes de ciertos participantes en las 10 disciplinas del decatlón

+ Si se fijan el nombre de la persona no es formalmente una columna del dataframe, es en realidad un índice o el nombre del renglón.

+ Hagamos que este nombre efectivamente sea una columna
...
+ Las columnas Rank, Points y Competition no nos interesan. Por el momento sólo nos interesan los puntajes de las 10 disciplinas.
...
+ Podemos ver que hay variables relacionadas fuertemente (ya sea positiva o negativamente) y otras que no tanto
...
+ **Importantísimo:** El parámetro scale siempre debe ser TRUE... pues generalmente las columnas originales están en diferentes unidades de magnitud. Se escalan las variables para poder compararlas entre sí
...
+ Con las primeras 4, el 80.2%... con este nivel a mí Lalo me es suficiente subjetivamente (por supesto), en función de mi experiencia en el fenónemo, alguna recomendación de la literatura para el fenómeno o simplemente mi capricho.
...
donde $\lambda_k$ es el $k$-ésimo eigenvalor.
...
+ En este caso, la sugerencia visual del gráfico de codo es quedarnos con 2 componentes principales
...
+ Voy a ver cómo se ven estos 4 puntos en el nuevo sistema coordenado
...
+ Nosotros además contamos con la variable de competencia (decastar y juegos olimpicos), la puedo incluir visualmente en mi análisis
...
+ ¿Cómo formo ese grupito? Hay varias formas, pero ahora sólo veremos k medias, i.e. se formarán k grupitos, de tal forma que la "variabilidad" en cada grupito sea lo más pequeña posible... i.e. estos grupitos sean lo más homogeneos internamente

+ Un plus es que sean muy diferentes externamente, i.e. que entre grupitos sean muy diferentes.
...
datos_con_cluster |> ggplot() +
  geom_point(aes(x = Murder, y = Assault,
                 color = cluster,
                 size = UrbanPop)) +
  geom_text_repel(aes(x = Murder, y = Assault,
                 label = estado,
                 color = cluster),
                 size = 3) +
  theme_light()
...
datos_con_cluster |> ggplot() +
  geom_point(aes(x = Murder, y = Rape,
                 color = cluster,
                 size = UrbanPop)) +
  geom_text_repel(aes(x = Murder, y = Rape,
                 label = estado,
                 color = cluster),
                 size = 3) +
  theme_light()
